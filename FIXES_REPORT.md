...
- The transactions pipeline only exported trades from `data/trades-*.json`, so adds/drops never reached `public/data/transactions`.
...
- `scripts/build_site_weekly_chunks.py`: now merges add/drop transactions from `data/{season}.json` when present (keeps trades from `data/trades-*.json`) and avoids emitting raw player IDs in summaries.
...
- Transactions: `public/data/transactions/{season}.json` generated from `data/trades-*.json` plus `data/{season}.json` `transactions` when available.
...
- `node scripts/verify_frontend_data.mjs`
  ...
  Transactions - trades: 44, adds: 0, drops: 0
  ```

Notes: adds/drops remain zero because no add/drop transactions exist in current `data/{season}.json` sources. The pipeline now supports them when present.
