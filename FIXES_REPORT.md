...
- `scripts/pull_espn_transactions.py`: fetches ESPN transactions into `data/espn-transactions-2025.json` when credentials are provided.
- `scripts/build_site_weekly_chunks.py`: now merges Sleeper + ESPN transactions (adds/drops/trades) and avoids emitting raw player IDs in summaries.
...
- Transactions: `public/data/transactions/{season}.json` generated from `data/trades-*.json` plus Sleeper `data/transactions-{season}.json` and ESPN `data/espn-transactions-{season}.json` when available.
...
- `node scripts/verify_frontend_data.mjs`
  ...
  ```
  ```
  ESPN transactions verify (2015-2024)
  - 2024 total: 1145
  - Types: WAIVER 542, FUTURE_ROSTER 172, DRAFT 152, TRADE_PROPOSAL 119, TRADE_DECLINE 75, ROSTER 66, TRADE_ACCEPT 9, TRADE_UPHOLD 9, TRADE_VETO 1
  ```
