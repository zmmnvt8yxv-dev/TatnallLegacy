...
exports.handler = async (event) => {
  // Simple protection: require ?key=ADMIN_TOKEN
  ...
  // fetch manifest
  const m = await fetch(`${RAW_BASE}/manifest.json`).then(r => r.json());
  const years = Array.isArray(m.years) ? m.years : [];

  // clear any old rows for these years (safe reimport)
  ...
  for (const y of years) {
    ...
    for (const tx of (doc.transactions || [])) {
      const { rows: txr } = await client.query(`INSERT INTO transactions(year,date_text) VALUES($1,$2) RETURNING id`, [y, tx.date || null]);
      const txId = txr[0].id;
  ...
  await client.end();
  return { statusCode: 200, body: JSON.stringify({ imported_years: years }) };
};
