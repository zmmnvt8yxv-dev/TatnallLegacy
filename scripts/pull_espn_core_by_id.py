...
def main():
    ...
    ap.add_argument("--start", type=int, default=0, help="0-based row offset into the CSV ids list")
    ap.add_argument("--limit", type=int, default=0, help="how many ids to process from start (0 = all)")
    ap.add_argument("--min-delay", type=float, default=0.25)
    ap.add_argument("--max-delay", type=float, default=0.75)
    ap.add_argument("--timeout", type=float, default=20.0)
    ap.add_argument("--resume", action="store_true", help="skip ids already in log as ok/404")
    args = ap.parse_args()
        ...
        for espn_id in tqdm(ids, desc="Fetch ESPN core athletes by id"):
            ...
            try:
                ...
                if http_status == 200 and body.strip():
                    # save json (or raw if ESPN ever returns non-json)
                    payload = {
                        "meta": {
                            "source": url,
                    ...
                    # do not save a file for 404; log it
