...
def fetch_json(session: requests.Session, url: str, params: Dict[str, Any], retries: int = 6, backoff: float = 1.4):
    ...
    for i in range(retries):
        try:
            ...
            # ESPN sometimes returns HTML on edge errors; keep it explicit
...
def main():
    ...
    ap.add_argument("--limit", type=int, default=200000, help="Value passed to ?limit= (ESPN pageSize-ish).")
    ap.add_argument("--outdir", type=str, default="data_raw/espn_core/index", help="Output directory")
    ap.add_argument("--resume", action="store_true", help="Skip already-downloaded pages")
    ap.add_argument("--max-pages", type=int, default=0, help="0 = no cap; otherwise stop after N pages")
    args = ap.parse_args()
    ...
    page_index = 1  # ESPN endpoints frequently accept page= or pageIndex=; your saved file had pageIndex.
    page_count: Optional[int] = None

    # We’ll try both param styles; whichever returns pageIndex/pageCount and items works.
    param_styles = [
        ("page", "limit"),
        ("pageIndex", "pageSize"),
    ]
    ...
    # First request: pick the style that works
    ...
    # If neither style worked, try a plain call with limit only (some servers ignore paging params)
    if chosen_style is None:
        ...
        if not isinstance(items, list) or not items:
            raise SystemExit("No items[] found. Open saved JSON in data_raw/espn_core/index/pages and inspect.")
        all_items = items
        page_count = 1
        chosen_style = ("(none)", "(none)")

    # Continue paging if pageCount exists and > 1
    if page_count and page_count > 1:
        p_page, p_size = chosen_style
        # We already downloaded page 1 above
        ...
        for _ in tqdm(range(2, max_pages + 1), desc="Fetching index pages"):
            ...
            if isinstance(items, list) and items:
                ...
                # If an empty page shows up, stop
                break
    ...
    # active subset (if field exists)
    if "active" in df.columns:
        df[df["active"] == True].to_csv(active_csv, index=False)
    else:
        # still write it, but empty
