...
interface Matchup {
    ...
    home_team?: string;
    away_team?: string;
    home_roster_id?: number | string;
    away_roster_id?: number | string;
...
export default function MatchupDetailPage(): React.ReactElement {
    const { season, week, matchupId } = useParams<{ season: string; week: string; matchupId: string }>();
    const { loading: contextLoading, error: contextError, playerIndex, teams, espnNameMap, playerSearch } = useDataContext() as {
        loading: boolean;
        error: string | null;
    ...
    const {
        ...
        isError: dataError
    } = useMatchupDetail(season, week) as {
        weekData: WeekData | undefined;
        fullStatsRows: LineupRow[];
        isLoading: boolean;
        isError: boolean;
    };
    ...
    const matchup = useMemo((): Matchup | undefined => {
    ...
    const getLineupTeamKeys = (matchupSide: string | undefined, rosterId: number | string | undefined): Set<string> => {
    ...
    const getMatchupLabel = (teamValue: string | undefined, rosterId: number | string | undefined): string => {
    ...
    const buildRoster = (teamKeys: Set<string>): RosterData => {
        ...
        const mapped = rows.map((row, originalIndex): EnrichedRow => {
        ...
        const positionalTotals = sortedRows.reduce((acc: Record<string, number>, row) => {
