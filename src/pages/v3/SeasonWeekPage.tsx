...
function positionTotals(lineup: WeeklyLineup): Map<string, number> {
  ...
  lineup.players.forEach((player) => totals.set(player.position || "—", (totals.get(player.position || "—") || 0) + player.projectedPoints));
  return totals;
}
...
export default function SeasonWeekPage(): React.ReactElement {
  ...
  const weeklyPlayers = useMemo(() => {
    ...
    return hub.data.teams.flatMap((team) => {
      ...
      return (lineup?.players || []).map((player) => ({ ...player, fantasyTeam: team }));
  ...
  return (
    <PageTransition>
      ...
      <section className="week-report-hero">
        <div><div className="live-line"><span /> Regular season · 4 matchups</div><Eyebrow>2026 weekly projection book</Eyebrow><h1>Week {String(weekNumber).padStart(2, "0")}</h1><p>The full Sleeper slate, thirteen projected starters per team, every positional edge and the upset condition in plain English.</p></div>
      ...
      <section className="forecast-section week-matchup-reports">
        ...
        {week.matchups.map((matchup) => {
          ...
          const underdogStars = [...underdogLineup.players].sort((a, b) => b.projectedPoints - a.projectedPoints).slice(0, 2);
          ...
          return <article className="week-matchup-report" id={`matchup-${matchup.matchupId}`} key={matchup.matchupId}>
            ...
            <div className="matchup-position-edges"><Eyebrow>Projected positional edges</Eyebrow>{positions.map((position) => { const a = totalsA.get(position) || 0; const b = totalsB.get(position) || 0; return <div key={position}><b>{position}</b><span className={a > b ? "won" : ""}>{a.toFixed(1)}<small>{teamA.monogram}</small></span><i><em style={{ left: `${Math.min(a / Math.max(a + b, 1) * 100, 100)}%` }} /></i><span className={b > a ? "won" : ""}>{b.toFixed(1)}<small>{teamB.monogram}</small></span></div>; })}</div>
            <div className="matchup-lineup-comparison"><section><header><TeamIdentity team={teamA} compact /><strong>{lineupA.projectedPoints.toFixed(1)}</strong></header>{lineupA.players.map((player) => <Link to={`/players/${player.playerUid || player.sleeperId}`} key={`${player.slot}-${player.sleeperId}`}><span>{player.slot}</span><b>{player.name}</b><small>{player.position} · {player.nflTeam || "FA"}</small><strong>{player.projectedPoints.toFixed(1)}</strong></Link>)}</section><section><header><TeamIdentity team={teamB} compact /><strong>{lineupB.projectedPoints.toFixed(1)}</strong></header>{lineupB.players.map((player) => <Link to={`/players/${player.playerUid || player.sleeperId}`} key={`${player.slot}-${player.sleeperId}`}><span>{player.slot}</span><b>{player.name}</b><small>{player.position} · {player.nflTeam || "FA"}</small><strong>{player.projectedPoints.toFixed(1)}</strong></Link>)}</section></div>
      ...
      <section className="forecast-section weekly-player-board">
        <div className="archive-section-heading"><div><Eyebrow>Projected starter leaderboard</Eyebrow><h2>Week {weekNumber}'s top 40.</h2></div><p>This ranking includes only players selected by the optimizer into a legal starting lineup for their team this week.</p></div>
        <div className="forecast-table-wrap"><div className="forecast-table forecast-table--weekly-players"><div className="forecast-table__head"><span>RK</span><span>Player</span><span>Slot</span><span>Team / owner</span><span>Projection</span></div>{weeklyPlayers.slice(0, 40).map((player, index) => <Link to={`/players/${player.playerUid || player.sleeperId}`} className="forecast-table__row" key={`${player.fantasyTeam.rosterId}-${player.sleeperId}`}><span className="forecast-seed">{index + 1}</span><span><b>{player.name}</b><small>{player.position} · {player.nflTeam || "FA"}</small></span><span><b>{player.slot}</b><small>projected starter</small></span><TeamIdentity team={player.fantasyTeam} compact /><span><b>{player.projectedPoints.toFixed(1)}</b><small>Sleeper half-PPR</small></span></Link>)}</div></div>
      </section>
